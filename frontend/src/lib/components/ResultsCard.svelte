...
<div class="rounded-lg border border-border bg-card p-6 space-y-6">
	...
	<div class="flex items-start justify-between">
		<div class="flex-1">
			<div class="flex items-center gap-3 mb-2">
				<StatusIcon class={`h-8 w-8 ${getStatusColor(result.BlackListed)}`} />
				<div>
	...
	<div class="flex flex-wrap gap-4 text-sm">
		...
		<div class="flex items-center gap-2">
			<ValidIcon class={`h-4 w-4 ${result.Status ? 'text-success' : 'text-destructive'}`} />
	...
	{#if result.BlackListed && Object.keys(result.BlackList).length > 0}
		<div>
			...
			<div class="space-y-2">
				{#each Object.entries(result.BlackList) as [blacklist, ips] (blacklist)}
